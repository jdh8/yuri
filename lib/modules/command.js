...
class CommandModule extends Module {
  ...
  add (commands) {
    ...
    for (const command of commands) {
      ...
      // Group exists
      if (!this.client.groups.map.has(command.group)) {
        this.client.emit('error',
      ...
      this.map.set(command.name, command instanceof Command
        ? command
        : new Command(command)
      )
      // Add command to group
      this.client.groups.map.get(command.group).commands.add(command.name)
