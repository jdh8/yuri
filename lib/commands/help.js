...
module.exports = {
  ...
  run ({message, args}) {
    ...
    if (!args) {
      ...
      const group = client.groups.find(command.group).title
      ...
      `)
    } else if (client.groups.find(isGroup(args))) {
      // Group help
      const group = client.groups.find(isGroup(args))
      ...
      const possibleGroup = client.groups.find(args)
        ? oneLine`
